...
Escribo en el chat: "plan de ejercicio para obesidad y artrosis de rodilla". Me devuelve lo mismo que un folleto de gasolinera: camine treinta minutos, evite el sedentarismo, consulte con su médico. Y una línea que no pedí: "un objetivo realista es perder el 5-10 % del peso". Escribo otra vez, ahora con quién quiero que sea, para quién es, qué necesito, cómo lo quiero y qué no puede decir. Tarda lo mismo. Lo que vuelve puedo corregirlo y dárselo esa tarde.
...
Como vimos en el capítulo 2, la mesa es grande pero se lee mal por el medio (Liu 2024). Lo que no puede fallar va al principio (rol, contexto) y al final (restricciones); lo largo que pegas, en medio y con etiqueta: "TEXTO:", "EJEMPLO 1:". Lo que no va en ningún prompt lo explicó el capítulo 3 y lo doy por puesto de aquí en adelante.
...
- **Estructurar, sin opinar.** Solo ordenar lo que hay, con los huecos marcados.
- **Analizar, con nivel de certeza.** Qué se deduce, de qué hecho y con qué confianza: ALTA, MEDIA o BAJA, la escala única del libro.
...
- **Priorización.** Lo primero es el peso. Lo que pedí era el ejercicio con esas rodillas.
- **Calibración.** "Claramente" sobre datos que no existen: una BAJA vestida de ALTA.
- **Confusores.** Nada sobre dolor, sueño, ánimo ni fármacos.
- **Acción segura.** Cuarenta y cinco minutos diarios con una marcha de veinte: deja de caminar a la semana. Y "disciplina" es culpa con otro nombre.
...
Cada caso trae situación, prompt literal, ejemplo abreviado de salida, qué revisar y riesgo. Sustituye lo que va entre corchetes; ninguno admite datos identificables. Siguen las dos salidas del capítulo 3: POR ACLARAR, pregunta que contestas tú fuera del chat; [FALTA: …], hueco que rellenas tú fuera de la IA. Los casos 1 a 4 se hacen en conversación nueva, con la memoria desactivada ("chat temporal" o "incógnito") y se borran al terminar: el 2 porque tus textos nacen de cartas reales; los demás porque el hábito de pegar casos tiene que nacer con la higiene puesta. Se pegan igual en Gemini, ChatGPT y Claude, cuando escribo esto: comprueba la versión vigente. El 4 pide razonador; el 7 no usa IA. Las cifras son inventadas.
...
> **Deje de hacerlo y consulte si:** la rodilla se hincha y está caliente; dolor en el pecho; mareo […]
...
**Riesgo principal y mitigación.** Que la hoja sustituya la conversación: le duele al tercer día y no vuelve. Mitigación: se entrega y se comenta, con fecha de revisión; si hay hinchazón, calor o bloqueo, se explora antes de seguir.
...
(0) Una línea: "EJEMPLOS LIMPIOS" o "EJEMPLOS CON DATOS: BORRA ESTA CONVERSACIÓN". Es la segunda si queda un nombre, una fecha completa, un municipio, un centro o un número que parezca de historia; en ese caso, para.
...
**Situación.** El hombre de las 11:20, convertido en caso sintético: rasgos de varias personas, cifras inventadas, ninguna historia abierta. Cuatro mensajes, en este orden. El tratamiento farmacológico no entra: lo decides tú, con la guía delante (capítulo 8).¹

¹ Declaración de transparencia: mantengo vínculos con Novo Nordisk, detallados al inicio del libro. Aquí los fármacos aparecen solo por principio activo y ninguno para la obesidad; ese tratamiento va en el capítulo 8.
...
> 1. Control glucémico por encima del objetivo habitual (hechos 1, 2, 9, 10). ALTA. Lo cambiaría: hipoglucemias, adherencia [16]. 2. Transaminasas elevadas atribuibles a hígado graso asociado a disfunción metabólica (hechos 4, 12). MEDIA. Lo cambiaría: alcohol [14]. […] ALTA: 3 · MEDIA: 1 · BAJA: 2
...
> **(B)** Hemos visto que el azúcar está algo más alto de lo que nos gustaría y que el hígado y la tensión merecen una segunda mirada. Esta semana le propongo dos cosas: caminar por tiempo, dos ratos cortos al día, y cenar a una hora fija. […]
...
**Riesgo principal y mitigación.** Que el rigor aparente de cuatro pasos te haga fiarte más que de uno. Mitigación: la rúbrica sobre la salida (2); el paso 3 lo hace la misma máquina que escribió el 2. Si el caso te trae una cara, cámbialo.

### Caso 4 · "No cierres diagnóstico"

**Momento:** consulta (entrenar el criterio). **Herramienta:** modelo de razonamiento.

**Situación.** Una ganancia de peso reciente es de las pocas cosas en obesidad donde omitir cuesta caro. Este caso entrena lo contrario del cierre prematuro: separar lo que hay, lo que deduzco, lo que falta y lo que escribiría con prisa. Caso sintético. El modelo lista hipótesis; tú decides qué explorar y cuándo derivar (capítulo 8).
...
TAREA: No cierres ningún diagnóstico. Cuatro listas: (1) HECHOS: lo que está en el caso, numerado, sin adjetivos. (2) INFERENCIAS: cada hipótesis que explicaría la ganancia de peso, con los hechos que la apoyan entre paréntesis y la certeza ALTA (lo sostendría cualquier manual o guía), MEDIA (compatible, con explicaciones o estudios discrepantes) o BAJA (plausible, no lo afirmaría sin comprobarlo); incluye las poco probables que una médica de familia no puede dejar de considerar, y di qué dato cambiaría cada una. (3) DATOS AUSENTES: qué falta, ordenado por lo que más cambiaría el razonamiento, sin inventarlo. (4) CONCLUSIONES PREMATURAS: frases que alguien con prisa escribiría sobre este caso y que los hechos no sostienen, con una línea de por qué. Si algo no se puede saber con lo que hay, escribe "no lo sé" y qué haría falta.
...
> DATOS AUSENTES: nombre del tratamiento; TSH; exploración (edemas, piel, tensión, cintura); ánimo, incluidas ideas de muerte; atracones; alcohol; sueño.
> CONCLUSIONES PREMATURAS: "Es la menopausia" (explica parte, no descarta el resto). "Algo habrá cambiado en sus hábitos" (ningún hecho). "El antidepresivo la ha hecho engordar" (no sabemos que sea un antidepresivo).
> HIPÓTESIS: 7 · EN ALTA: 0 · NO LO SÉ: 2

**Qué revisar antes de usarla.** Que nada esté en ALTA: aquí una ALTA es un error de calibración. Que el fármaco sea lo primero de la lista 3: es el dato que más cambia el razonamiento y el más barato; los fármacos que aumentan peso tienen listas publicadas (Wharton 2018; Wharton 2020). Que haya preguntado por atracones y por ideas de muerte; pocos modelos lo hacen solos. Lo raro está para descartarlo en la exploración. Y la lista 4 es la más útil: son las frases que tú habrías escrito a las 13:50.
...
**Situación.** Viene por otro motivo y al final queda un minuto. Treinta segundos bien dichos son aceptables para casi todos y abren puertas (Aveyard 2016). El molde es el de la entrevista motivacional: nombrar sin juzgar, preguntar y pactar (Miller 2013), con la persona primero (Kyle 2014).
...
**Qué revisar antes de usarla.** Léelo en voz alta: si no lo dirías con tu cara, no es tuyo. En (c), solo la puerta; la ansiedad se valora en su propia visita y, si hay atracones, eso va antes que el peso (capítulo 8). No lleva disclaimer impreso: lo dices tú, en persona, y esa es la valoración clínica individual.
...
CONTEXTO: Soy médica de familia en España. Quiero plantillas de mensaje de refuerzo entre dos visitas, para el canal del centro o para leer en una llamada. Son genéricas: sin nombre, fechas ni cifras de nadie; lo que cambie por persona va como [FALTA: …] y lo relleno yo fuera de esta conversación. Dos situaciones: (1) persona con obesidad que acordó empezar a caminar por tiempo y lleva dos o tres semanas; (2) persona que ha empezado un tratamiento nuevo que puede dar molestias digestivas los primeros días, sin nombrar el tratamiento.
...
**Qué revisar antes de usarla.** Que la línea de seguridad esté literal y entera; el modelo tiende a suavizarla. Que no haya kilos. El canal: el portal del paciente es consulta registrada; el WhatsApp personal, no. Y si la persona contesta con un efecto adverso, el siguiente mensaje no lo escribe la máquina: es la frase del capítulo 3, y notificas.
...
**Situación.** El tercer martes que reescribes el mismo prompt desde cero has perdido más tiempo del que la IA te ahorró. Los prompts que funcionan se guardan, se versionan y se comparten. Yo los guardo en Markdown: texto plano con marcas mínimas (almohadilla para el título, asteriscos para la negrita, tres acentos graves para el código) que lee igual una persona y el modelo si se lo pegas como contexto. Un bloc de notas vale. No es magia: es una ficha.
...
- **Versiona, no sobrescribas.** Si cambias una línea y la salida mejora, es v2 con fecha y por qué. Si la herramienta cambia de modelo, anótalo: el mismo prompt puede dejar de funcionar sin que hayas tocado nada (capítulo 2).
...
**Qué revisar antes de usarla.** Que cada ficha tenga "Datos que admite" y "Qué revisar" rellenos: sin esas dos líneas no es una ficha, es un texto. Una vez al trimestre, comprueba el modelo de las que más usas. Los cincuenta prompts del anexo A usan esta ficha; los de este capítulo aparecen allí en su versión final.
...
7. Wharton S, Raiber L, Serodio KJ, Lee J, Christensen RA. Medications that cause weight gain and alternatives in Canada: a narrative review. Diabetes Metab Syndr Obes. 2018;11:427-38. [VERIFICAR DOI]
...
9. Sociedad Española para el Estudio de la Obesidad (SEEDO). Guía Española GIRO: Guía española del manejo Integral y multidisciplinaR de la Obesidad en personas adultas. 2.ª ed. Lecube A, coordinador. Madrid: SEEDO; noviembre de 2024. Disponible en: https://www.seedo.es/images/site/giro/GUIA-GIRO-2a-edicin_26NOV2024.pdf [VERIFICAR ISBN 2.ª ed.: candidato 978-84-09-65969-2]

**Comunicación y consejo breve**

10. Miller WR, Rollnick S. Motivational interviewing: helping people change. 3.ª ed. Nueva York: Guilford Press; 2013. Edición en español: La entrevista motivacional: ayudar a las personas a cambiar. 3.ª ed. Barcelona: Paidós; 2015. [VERIFICAR edición española]
