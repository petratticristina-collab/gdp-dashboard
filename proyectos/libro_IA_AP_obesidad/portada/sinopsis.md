# Portada, sinopsis y tapas · *IA en la consulta: la revolución que cabe en diez minutos*

> Preparado el 22-09-2026 por el orquestador. Los textos son propuesta para la autora; los huecos [POR ACLARAR] los rellena ella. Los archivos `tapa.pdf`, `contratapa.pdf` y `tapas_completas.pdf` se generan con `build_tapas.py` (formato 15 × 23 cm; lomo estimado [POR ACLARAR: según páginas finales y papel]).

## Tapa (portada)

**Título:** IA en la consulta
**Subtítulo de cubierta:** la revolución que cabe en diez minutos
**Línea de posicionamiento:** Aplicación práctica para el médico de familia ante la obesidad
**Autora:** Dra. Cristina B. Petratti
**Línea inferior:** Prólogo de [POR ACLARAR: nombre y cargo] · Continuación profesional de *Obesidades sin culpa*

## Contratapa (sinopsis)

**Frase de apertura (grande):**
Treinta y cuatro pacientes, seis huecos de urgencias, dos domicilios y siete minutos por persona. Ahí es donde la inteligencia artificial tiene que caber. Si no cabe ahí, no sirve.

**Sinopsis (unas 170 palabras):**
Este libro no promete que una máquina vaya a ver a tus pacientes por ti. Promete algo más útil: devolverte minutos y enseñarte qué hacer con ellos cuando la persona que tienes delante lleva veinte años oyendo "coma menos y muévase más".

En diez capítulos, una médica de familia con más de veinticinco años de consulta cuenta cómo usa hoy la IA generativa ante la obesidad, sin meter ningún dato de nadie en ninguna herramienta: la hoja para la paciente en su idioma, el asistente que cita las guías y no decide, la calculadora que construyó sin saber programar, el guion del reel que pasa una auditoría antes de publicarse, la notificación de un efecto adverso, la hoja de ruta de noventa días. Más de setenta casos de uso, cada uno con su prompt listo para pegar, su ejemplo, lo que hay que revisar antes de usarlo y el riesgo que conviene conocer.

Todo con una idea en el centro, la misma que en *Obesidades sin culpa*: no es falta de voluntad, es biología. La máquina te devuelve los minutos. Lo que cabe en ellos, con la persona delante, lo haces tú.

**Tres líneas de qué encontrarás:**
- Prompts para consulta, seguimiento, burocracia, docencia y divulgación, sin datos de pacientes.
- Lo que sale mal y cómo pararlo antes de que llegue a la persona: sesgo de peso, alucinaciones, exceso de seguridad.
- Un plan de noventa días y una comunidad de práctica para no aprender esto sola.

**Biografía de la autora (unas 80 palabras):**
Cristina B. Petratti es médica de familia, especialista en obesidad y salud metabólica, con más de veinticinco años de consulta. Miembro de la SEEDO (Grupo de Trabajo de Ejercicio Físico y Obesidad; coautora de SEEDO GO!), creadora del Método Petratti y autora de *Obesidades sin culpa* (2026). Se formó en 2026 en un programa estructurado de inteligencia artificial generativa para profesionales sanitarios y en compliance para la divulgación científica digital. Corrió la maratón de Londres en 2025. Ejerce en Alicante.

**Declaración breve (pie de contratapa):**
La autora declara vínculos con la industria farmacéutica, detallados al inicio del libro. Ningún medicamento de prescripción aparece con nombre comercial.

**Elementos técnicos:** ISBN [POR ACLARAR] · código de barras · editorial o autoedición [POR ACLARAR] · precio [POR ACLARAR] · logotipo Método Dra. Petratti (archivo pendiente: enviar PNG) · foto de la autora [POR ACLARAR].

## Lomo

IA en la consulta · Cristina B. Petratti · [logo]

## Solapas (si la edición las lleva)

**Solapa delantera:** la biografía larga y la foto.
**Solapa trasera:** *Obesidades sin culpa* (2026): "Dieciocho capítulos para pacientes; este libro es su continuación para el profesional."

## Sintaxis de cubierta (jerarquía tipográfica propuesta)

1. Título, muy grande, dos líneas: "IA en la consulta".
2. Subtítulo en cursiva, una línea: "la revolución que cabe en diez minutos", con "diez minutos" enlazado al motivo gráfico (un reloj con un sector de diez minutos).
3. Línea de posicionamiento, pequeña, en mayúsculas espaciadas.
4. Autora, tamaño medio, con "Dra." en peso ligero.
5. Prólogo y pertenencia, pie de cubierta.
Colores de marca: verde azulado #1AA5A5, verde #8DC63F, verde claro #C5D96B, gris #8A9299; fondo blanco o verde azulado profundo #0E5F63 según variante.
